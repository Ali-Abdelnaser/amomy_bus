...
void main() {
  ...
  group('AMOMY Google Maps Production Migration Tests', () {
    testWidgets('1. Embedded monochrome style JSON is valid, light, and hides clutter', (tester) async {
      final jsonStr = await rootBundle.loadString('assets/map/amomy_google_map_style.json');
      final dynamic parsed = jsonDecode(jsonStr);
      expect(parsed, isA<List>());

      // Verify exact Silver palette colors are present
      expect(jsonStr.contains('#F4F5F7'), isTrue, reason: 'Map background #F4F5F7');
      expect(jsonStr.contains('#FFFFFF'), isTrue, reason: 'Major roads #FFFFFF');
      expect(jsonStr.contains('#E9ECF0'), isTrue, reason: 'Minor roads #E9ECF0');
      expect(jsonStr.contains('#D5DAE0'), isTrue, reason: 'Road borders #D5DAE0');
      expect(jsonStr.contains('#667085'), isTrue, reason: 'General labels #667085');
      expect(jsonStr.contains('#98A2B3'), isTrue, reason: 'Secondary labels #98A2B3');
      expect(jsonStr.contains('#E3E7EB'), isTrue, reason: 'Water #E3E7EB');
      expect(jsonStr.contains('#ECEFF2'), isTrue, reason: 'Parks #ECEFF2');
      expect(jsonStr.contains('#F1F3F5'), isTrue, reason: 'Landscape #F1F3F5');
      expect(jsonStr.contains('#D8DDE3'), isTrue, reason: 'Administrative boundaries #D8DDE3');

      // Verify no vivid green/red park styling (parks muted/desaturated)
      expect(jsonStr.contains('#00FF00'), isFalse);
      expect(jsonStr.contains('#4CAF50'), isFalse);

      // Verify POI clutter visibility is off
      expect(jsonStr.contains('"visibility": "off"'), isTrue);
    });

    test('2. Custom Stop Pin icon generation creates valid BitmapDescriptors', () async {
    ...
    test('3. Bus Marker icon is circular, distinct, and does not depend on heading rotation', () async {
      final busLive = await AmomyMapIcons.getBusMarkerIcon(isStale: false, isQa: false);
      expect(busLive, isNotNull);

      final busStale = await AmomyMapIcons.getBusMarkerIcon(isStale: true, isQa: false);
      expect(busStale, isNotNull);

      final busQa = await AmomyMapIcons.getBusMarkerIcon(isStale: false, isQa: true);
      expect(busQa, isNotNull);
    });

    test('4. Route geometry polyline decoding handles Google Encoded format', () {
    ...
    test('5. Route validation calculates perpendicular/shortest stop-to-polyline distance', () {
      final polyline = [
      ...
      final closeStop = {'id': 's1', 'stop_order': 18, 'latitude': 30.9351, 'longitude': 31.3471};
      ...
      final farStop = {'id': 's2', 'stop_order': 1, 'latitude': 31.0000, 'longitude': 31.5000};
    ...
    test('7. Outbound and Return geometries are separate and non-reversible', () {
      ...
      expect(outbound.polylineEncoded, isNot(equals(returnGeom.polylineEncoded)));
    });

    test('8. ETrack GPS projection keeps raw GPS authoritative if off-route (> 80m)', () {
      final polyline = [
        const LatLng(30.9350, 31.3470),
        const LatLng(30.9388, 31.3524),
      ];
    ...
    test('10. Stop ETA calculates distance along polyline when road geometry is present', () {
      final polyline = [
        const LatLng(30.9350, 31.3470),
    ...
    test('11. AmomyMapIcons defines precise anchors aligning pin tip and bus center to GPS', () {
    ...
    test('14. Stop Pin semantics: Last is yellow, Next is blue, Future is white, Passed is muted', () async {
      await AmomyMapIcons.preloadAllIcons();

      // Ensure all descriptors generated without throwing
      final lastPin = AmomyMapIcons.getCachedStopIcon(StopPinVisualState.current);
      final nextPin = AmomyMapIcons.getCachedStopIcon(StopPinVisualState.next);
      final futurePin = AmomyMapIcons.getCachedStopIcon(StopPinVisualState.normal);
      final passedPin = AmomyMapIcons.getCachedStopIcon(StopPinVisualState.passed);
      final selLastPin = AmomyMapIcons.getCachedStopIcon(StopPinVisualState.selectedLast);
      final selNextPin = AmomyMapIcons.getCachedStopIcon(StopPinVisualState.selectedNext);
    ...
    test('15. Stop timing engine preserves actual recorded arrival timestamp and does not fabricate when null', () {
    ...
    test('16. Clock format displays exact passenger format in Arabic and English', () {
      ...
      expect(enFormatted.contains('03:42 PM') || enFormatted.contains('3:42 PM'), isTrue);
      expect(arFormatted.contains('03:42 م') || arFormatted.contains('3:42 م'), isTrue);
    });
  });
}
