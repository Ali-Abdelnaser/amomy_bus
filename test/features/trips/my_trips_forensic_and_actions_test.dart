...
class FakeBookingRepository implements BookingRepository {
  ...
  final bookingUpdatesController = StreamController<void>.broadcast();
  ...
  Stream<void> subscribeToPassengerBookingUpdates() =>
      bookingUpdatesController.stream;
}
...
void main() {
  ...
  group('PART T — MY TRIPS FORENSIC CONTRACT TESTS', () {
    test(
      '1. reinstall / local state clearing has no effect on backend bookings',
      () async {
        fakeRepo.bookings = [
          PassengerBooking(
            bookingId: 'bkg-1',
    ...
    test(
      '2 & 3. booked today trip appears Booked and available trip appears Available',
      () async {
    ...
    test(
      '4, 5 & 6. booked fare uses booking.fare_points, available fare uses boarding-stop fare with no 50-point fallback',
      () {
    ...
    test('8 & 9. past and cancelled bookings appear in History', () async {
      ...
      expect(
        cubit.state.historyTrips.any((b) => b.status == 'completed'),
        isTrue,
      );
      expect(
        cubit.state.historyTrips.any((b) => b.status == 'cancelled'),
        isTrue,
      );
    });

    test('booking realtime insert refreshes visible My Trips state', () async {
      await cubit.loadTripsHub();
      expect(cubit.state.status, PassengerTripsStatus.loaded);
      expect(cubit.state.upcomingTrips, isEmpty);

      final futureDeparture = DateTime.now().add(const Duration(hours: 2));
      fakeRepo.bookings = [
        PassengerBooking(
          bookingId: 'bkg-live',
          tripId: 'trip-live',
      ...
      fakeRepo.bookingUpdatesController.add(null);
      await pumpEventQueue();

      expect(cubit.state.upcomingTrips.length, 1);
      expect(cubit.state.upcomingTrips.single.bookingId, 'bkg-live');
    });
  });

  group('PART T — CANCEL TESTS', () {
    test(
      '10 & 11. cancel within T-30m succeeds and past T-30m fails',
      () async {
    ...
    test(
      '12, 13, 14 & 15. cancel frees seat, refunds points once, is idempotent and allows rebooking',
      () async {
  ...
  group('PART T — CHANGE SEAT TESTS', () {
    test(
      '16 & 17. change seat within cutoff succeeds, after cutoff fails',
      () async {
    ...
    test(
      '18, 19, 20, 21 & 22. seat change preserves seat if bus full, protects seat during modal, swaps atomically',
      () {
        final allSeats = [
          const TripSeat(
            seatId: 'seat-1',
