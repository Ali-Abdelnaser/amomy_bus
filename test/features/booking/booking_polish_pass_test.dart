...
class _FakeBookingRepository implements BookingRepository {
  ...
  }) async => Success(confirmedBooking!);
...
void main() {
  ...
  group('DEPARTURE TIME REDESIGN & FILTERING TESTS', () {
    ...
    testWidgets('1. departure options render vertically in a column', (
      tester,
    ) async {
    ...
    testWidgets('2. only one option selected at a time with radio behavior', (
      tester,
    ) async {
      TripOption? selected;
      await tester.pumpWidget(
    ...
    test('3. existing booked trip is removed from available options', () async {
      ...
      expect(
        cubit.state.availableTrips.any((t) => t.tripId == 'trip-0800'),
        isFalse,
      );
    });
    ...
    test(
      '4. cancelled booking does not block rebooking the same trip',
      () async {
        final repo = _FakeBookingRepository();
        repo.trips = [sampleTrip1, sampleTrip2];
        repo.passengerBookings = [
    ...
    testWidgets(
      '6. specific-trip flow displays locked selected time correctly',
      (tester) async {
        await tester.pumpWidget(
          buildTestableWidget(
            DepartureTimeSelector(
    ...
    testWidgets('7. no available trips shows proper empty state', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestableWidget(
  ...
  group('SEAT MAP & AVATARS TESTS', () {
    ...
    testWidgets('8. bus entrance starts below final position and completes', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestableWidget(
    ...
    testWidgets('9. animation does not replay on seat selection/rebuild', (
      tester,
    ) async {
      TripSeat? selected;
      await tester.pumpWidget(
    ...
    testWidgets('10. animation can replay on re-entry (fresh widget mount)', (
      tester,
    ) async {
      bool inSeatMap = true;
    ...
    testWidgets('11. reduced motion shows final bus position immediately', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestableWidget(
    ...
    testWidgets('12. initially booked male seat shows avatar on first frame', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestableWidget(
    ...
    testWidgets(
      '13. initially booked female seat shows avatar on first frame',
      (tester) async {
        await tester.pumpWidget(
          buildTestableWidget(
            const BusSeatVisual(
    ...
    testWidgets('14. held seat icon appears immediately on first frame', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildTestableWidget(
    ...
    testWidgets(
      '15. successful seat hold uses bottom banner, not center dialog',
      (tester) async {
        final repo = _FakeBookingRepository();
    ...
    testWidgets(
      '16. changing seat updates banner/hold cleanly without timer stacking',
      (tester) async {
        final repo = _FakeBookingRepository();
        repo.trips = [sampleTrip1];
