...
void main() {
  group('Supervisor Reserved Seat UI & Contract Tests', () {
    ...
    test('BusSeatLayoutConfig.resolveVisualState prioritizes supervisorReserved over selection/isMine', () {
      ...
      expect(visualStateWithSelection, equals(SeatVisualState.supervisorReserved));
    ...
    testWidgets('BusSeatVisual for supervisorReserved is non-tappable and does not fire onTap', (tester) async {
      bool tapped = false;
      ...
      expect(
        find.byWidgetPredicate(
          (w) => w is BusSeatVisual && w.state == SeatVisualState.supervisorReserved && w.label == '1',
        ),
        findsOneWidget,
      );
      ...
      expect(tapped, isFalse, reason: 'Supervisor reserved seat must never trigger onTap');
    });

    testWidgets('ProfessionalBusSeatMap renders Seat 1 beside driver and ignores taps on Seat 1', (tester) async {
      TripSeat? tappedSeat;
      ...
      expect(seat1Finder, findsOneWidget, reason: 'Seat 1 must remain physically visible');
      ...
      await tester.pump();
      expect(tappedSeat, isNull, reason: 'Tapping Seat 1 must not call onSeatTap');
