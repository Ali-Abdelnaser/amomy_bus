-- ============================================================================
-- AMOMY BUS - FIX SEAT HOLD STATUS ENUM IN CONFIRM_BOOKING & CREATE_BOOKING_HOLD
-- Migration: 20260912210000_fix_seat_hold_status_converted.sql
--
-- Root Cause:
-- PostgreSQL enum public.seat_hold_status is: ('active', 'released', 'expired', 'converted')
-- In 20260912200000_fix_point_holds_and_booking_schema_drift.sql,
-- confirm_booking() attempted to set seat_holds.status = 'fulfilled',
-- which violates the enum constraint (PostgrestException 22P02).
-- Also in create_booking_hold(), stale holds were updated to 'cancelled' (not in enum).
-- Furthermore, bookings insert referenced non-existent columns (seat_hold_id, point_hold_id).
--
-- This migration updates:
-- 1. confirm_booking(uuid): sets public.seat_holds.status = 'converted' and
--    uses authoritative bookings & point_transactions columns.
-- 2. create_booking_hold: cancels stale user holds with status = 'released' and
--    restores any previously held points to available balance.
-- ============================================================================
...
-- Update create_booking_hold to use 'released' instead of 'cancelled' for existing holds
