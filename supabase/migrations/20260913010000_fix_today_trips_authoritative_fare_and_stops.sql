-- =============================================================================
-- AMOMY BUS - FIX TODAY TRIPS AUTHORITATIVE FARE & STOPS PRESENTATION
-- Migration: 20260913010000_fix_today_trips_authoritative_fare_and_stops.sql
-- =============================================================================

-- 1. Ensure trips.fare_points defaults to 30 instead of 50
ALTER TABLE public.trips ALTER COLUMN fare_points SET DEFAULT 30;

-- 2. Update get_passenger_today_trips with authoritative fare and stop hierarchy
...
RETURNS TABLE(
  ...
  booking_close_at timestamp with time zone,
  fare_points numeric,
  total_seats integer,
...
BEGIN
  ...
  ELSIF v_user_id IS NOT NULL THEN
    -- Fetch passenger preferred stops
  ...
  default_fares AS (
    -- Authoritative default boarding stop (stop_order = 1) fare per route
  ...
  SELECT
    ...
    t.service_date,
    -- Origin presentation:
    -- 1. Booked trip: actual booked boarding stop
    -- 2. Outbound preference origin
    -- 3. Return preference origin (boarding at destination end)
    -- 4. Route default origin
    COALESCE(
    ...
    t.booking_close_at,
    -- Authoritative fare:
    -- Booked: frozen snapshot (b_fare_points)
    -- Available: explicit stop fare -> preferred boarding stop fare -> route default stop fare -> 30
    COALESCE(
      ub.b_fare_points,
      v_stop_fare,
