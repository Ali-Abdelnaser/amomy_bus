...
class _BookingReviewCardState extends State<BookingReviewCard>
  ...
  Widget build(BuildContext context) {
    ...
    final double outboundFare =
        (bundle != null && bundle.outboundBaseFarePoints > 0)
        ? bundle.outboundBaseFarePoints
        : (returnOption?.outboundBaseFarePoints ?? 0.0);

    final double returnFare =
        (bundle != null && bundle.returnBaseFarePoints > 0)
        ? bundle.returnBaseFarePoints
        : (returnOption?.returnBaseFarePoints ?? 0.0);

    final double subtotalPoints = (bundle != null && bundle.subtotalPoints > 0)
        ? bundle.subtotalPoints
        : (returnOption?.subtotalPoints ?? (outboundFare + returnFare));

    final int discountPercent = (bundle != null && bundle.discountPercent > 0)
        ? bundle.discountPercent.toInt()
        : (returnOption?.discountPercent.toInt() ?? 15);

    final double discountPoints = (bundle != null && bundle.discountPoints > 0)
        ? bundle.discountPoints
        : (returnOption?.discountPoints ??
              (subtotalPoints * (discountPercent / 100)));

    final double totalPoints = (bundle != null && bundle.totalPoints > 0)
        ? bundle.totalPoints
        : (returnOption?.totalPoints ?? (subtotalPoints - discountPoints));

    if (isBundle &&
        (outboundFare <= 0 || returnFare <= 0 || totalPoints <= 0)) {
      return Center(
        child: Padding(
